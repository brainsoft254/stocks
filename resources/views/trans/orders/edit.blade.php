							...
							<div class="form-group {{ $errors->has('discount') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control discount" name="discount" id="discount">
										...
										<option value="{{$disc->rate}}" {{($disc->rate==$order->discrate?'selected':'')}} >{{$disc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>	
								...
								<tbody class="order-body">
									...
									<tr id="tr_{{$i}}" class="line-item">
										...
										<td>
											...
											<select class='form-control-grid iqty ' name='iqty[]' id='iqty_{{$i}}' style='display:none;border-radius:0px !important;'>
...
<script type="text/javascript">
	$(document).ready(function(){
	   ...
	   $(document).on("change",".discount",function(event){
		...
		function calculateDisc()
		{
			...
			console.log("VatStat="+$('#vatstat').val());
	...
	/*		
		...
		function addRow(rid){
			...
			row+="</select>"+
			"<select class='form-control-grid iqty ' name='iqty[]' id='iqty_"+rid+"' style='display:none;border-radius:0px !important;'>"+
			"<option value='0' selected>0</option>";
			for(i in items){
