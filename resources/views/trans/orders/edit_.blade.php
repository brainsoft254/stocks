									...
									<tr id="tr_{{$i}}" class="line-item">
										<td>
											<select class='form-control-grid select2-selection-grid itemcode col-xs-4' name='itemcode[]' id='itemcode_{{$i}}' style='border-radius:0px !important;width:auto;'>
												...
												<option value="{{$item->code}}" {{$orderd->icode==$item->code?'selected':''}} >{{$item->code}}</option>
												@endforeach
											</select>
										</td>
										<td>
											<select class='form-control-grid select2-selection-grid description col-xs-6' name='description[]' id='description_{{$i}}' style='border-radius:0px !important;'>
												...
												@foreach($items as $item)
												<option value="{{$item->description}}"  {{Stockspro::getItemDescrClient($orderd->icode,$order->clcode)==$item->description? 'selected':''}}>{{$item->description}}</option>
												@endforeach
											</select>
											<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_{{$i}}' style='display:none;border-radius:0px !important;'>
												@foreach($items as $item)
												<option value="{{Stockspro::getClientPrice($item->code,$order->clcode)}}"  {{(Stockspro::getClientPrice($item->code,$order->clcode)==$orderd->rate)?'selected':''}} >{{Stockspro::getClientPrice($item->code,$order->clcode)}}</option>
												@endforeach
											</select>
											<select class='form-control-grid iqty col-xs-6' name='iqty[]' id='iqty_{{$i}}' style='display:none;border-radius:0px !important;'>
												<option value='0' selected>0</option>
												@foreach($items as $item)
												<option value='{{Stockspro::getItemQty($item->code,$order->location)}}'>{{Stockspro::getItemQty($item->code,$order->location)}}</option>
												@endforeach
											</select>
										</td>
										<td>
											<select class='form-control-grid uom  col-xs-4' name='uom[]' id='uom_{{$i}}' style='border-radius:0px !important ;width: auto' >
												...
												<option value="{{$uom->code}}" {{$orderd->uom==$uom->code?'selected':''}}>{{$uom->description}}</option>
												@endforeach
											</select>
...
<script type="text/javascript">
	...
	/*		
		...
		function addRow(rid){
			...
			@foreach($items as $item)
			"<option value='{{trim(Stockspro::getItemDescrClient($item->code,"+$('#client').val()+"))}}'>{{trim(Stockspro::getItemDescrClient($item->code,"+$('#client').val()+"))}}</option>"+
			@endforeach
			"</select>"+
			"<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_"+rid+"' style='display:none;border-radius:0px !important;'>"+
			"<option value='0' selected>0</option>"+
			@foreach($items as $item)
			"<option value='{{Stockspro::getClientPrice($item->code,"+$('#client').val()+")}}'>{{Stockspro::getClientPrice($item->code,"+$('#client').val()+")}}</option>"+
			@endforeach
			"</select>"+
