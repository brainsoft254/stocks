	...
	<div class="col-md-10">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form class="form-order" action="{{route('requisitions.update',$requisition->id)}}" method="POST">
					@csrf @method('put')
					<div class="row">
						<div class="col-md-3">
							<div class="form-group {{ $errors->has('refno') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="refno" class="form-control" value="{{$requisition->refno}}"  placeholder="Enter Sales Order No." readonly>
								</div>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group {{ $errors->has('trandate') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="trandate" value="{{$requisition->trandate}}" class="form-control trandate" id="trandate" placeholder="Trandate" readonly="">
								</div>
							</div>
						</div>
						<div class="col-md-5">
							<div class="form-group {{ $errors->has('locfrom') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control" name="locfrom" id="locfrom">
										...
										<option value="{{$loc->code}}" {{$loc->code==$requisition->locfrom?"selected":''}}>{{$loc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group {{ $errors->has('locto') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control locto" name="locto" id="locto">
										...
										<option value="{{$loc->code}}" {{$loc->code==$requisition->locto?"selected":''}}>{{$loc->description}}</option>
										@endif
										@endforeach
									</select>
								</div>
						...
						<div class="col-md-12">
							<div class="form-group {{ $errors->has('remarks') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<textarea class="form-control" name="remarks" id="remarks" rows=1 style="resize: none;">{{$requisition->remarks}}</textarea>
								</div>
							</div>
						</div>
					</div>
						...
						<div class = "col-md-12">
							...
							<table class="table-items" style="width:100%;" >
								...
								<tbody class="order-body">
									<?php $i=1;?>
									@foreach($requisition->req_details as $reqd)
									<tr id="tr_{{$i}}" class="line-item">
										<td>
											<select class='form-control-grid select2-selection-grid itemcode col-xs-4' name='itemcode[]' id='itemcode_{{$i}}' style='border-radius:0px !important;width:auto;'>
												<option value="{{$reqd->icode}}" selected >{{$reqd->icode}}</option>
											</select>
										</td>
										<td>
										...
										<td>
											<select class='form-control-grid uom  col-xs-4' name='uom[]' id='uom_{{$i}}' style='border-radius:0px !important ;width: auto' >
												...
												<option value="{{$reqd->uom}}" {{$reqd->uom==$uom->code?'selected':''}}>{{$uom->description}}</option>
												@endforeach
											a							</select>
											<select class='form-control-grid factor  col-xs-4' name='factor[]' id='factor_{{$i}}' style='display:none;border-radius:0px !important ;width: auto' >
												@foreach($units as $uom)
												<option value='{{$uom->factor}}' {{$reqd->uom==$uom->code?'selected':''}}>{{$uom->factor}}</option>
												@endforeach
											</select>
										</td>
										<td><input type="text" name="qty[]" id="qty_{{$i}}" class="text-center  form-control-grid qty  col-xs-2 " value="{{$reqd->qty}}" ></td>
										<td><input type="text" name="price[]" id="price_{{$i}}" class="form-control-grid price col-xs-2" value="{{$reqd->rate}}" readonly></td>
										<td>
											<select class="form-control-grid vatable col-xs-6 text-danger" name="vatable[]" id="vatable_{{$i}}"'>
												<option value="1" {{$reqd->vat>0?"selected":""}}>yes</option>
												<option value="0" {{$reqd->vat==0?"selected":""}}>No</option>
											</select>
										</td>										
										<td>
											<input type="text" id="vat_{{$i}}" value="{{$reqd->vat}}" name="vat[]" class="vat form-control-grid vat col-xs-2 text-right" value="{{$requisition->tax}}" readonly="">
										</td>
										<td>
											<input type="text" id="total_{{$i}}" value="{{$reqd->total}}" name="total[]"  value="{{$reqd->total}}" class="total form-control-grid col-xs-3 text-right" readonly="">
										</td>
										<td>
						...
						<div class="col-md-8">
							<div class="col-md-4 form-group {{ $errors->has('vattotal') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="tqty" class="tqty form-control text-right" value="{{$requisition->qty}}" readonly="" >
								</div>
							</div>
							<div class="col-md-4 form-group {{ $errors->has('vattotal') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon warning">Vat Total</span>
									<input type="text" name="vattotal" class="vattotal form-control text-right" value="{{$requisition->tax}}" readonly="" >
								</div>
							</div>
							<div class="col-md-4  form-group {{ $errors->has('grosstotal') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon danger">Gross Total</span>
									<input type="text" name="grosstotal" class="grosstotal form-control text-right" value="{{$requisition->total}}" readonly="" >
								</div>
							</div>
...
<script type="text/javascript">
	$(function(){
		...
		function addRow(rid){
			...
			var items={!!$items!!};
			if($('#locto').val()==-1){swal("","Select Location To & Try Again","error");return;}
			...
			row+="</select>";
			row+="<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_"+rid+"' style='display:none;border-radius:0px !important;'>"+
			...
			"<option value='0' selected>0</option>";
			for(i in items){
				row+="<option value='"+items[i].iqty+"'>"+items[i].iqty+"</option>";
			}
			row+="</select>"+
			"</td>"+
			"<td>"+
