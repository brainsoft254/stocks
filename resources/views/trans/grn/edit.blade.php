	...
	<div class="col-md-12">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="text-default"><i class="fa fa-magic "></i> <strong> Edit Goods Received Note</strong></h3>
			</div>
			<div class="panel-body">
				<form class="form-grn" action="{{route('grn.update',$grn->id)}}" method="POST">
					@csrf @method('PUT')
					<div class="row">
						<div class="col-md-6">
							<div class="form-group {{ $errors->has('status') ? ' has-error' : '' }}" id="status-group">
								<fieldset class="the-fieldset-success">
									...
									<div class="form-radio abc-radio abc-radio-success col-xs-6">
										<input type="radio" id="purchases" value="0" name="receivemode" {{$grn->trantype==0?'Checked':''}} class="receivemode">
										<label for="purchases">Purchases</label>
									</div>
									<div class="form-radio abc-radio abc-radio-success col-xs-6"">
										<input type="radio" id="opstock" value="1" name="receivemode" class="receivemode" {{$grn->trantype==1?'Checked':''}}>
							...
							<div class="form-group {{ $errors->has('status') ? ' has-error' : '' }}" id="status-group">
								<fieldset class="the-fieldset">
									...
									<div class="form-radio abc-radio abc-radio-primary col-xs-6">
										<input type="radio" id="cash" value="0" class="ptype" name="ptype" {{$grn->pmode==0?'Checked':''}}>
										<label for="cash">Cash</label>
									</div>
									<div class="form-radio abc-radio abc-radio-primary col-xs-6"">
										<input type="radio" id="credit" value="1" class="ptype" name="ptype" {{$grn->pmode==1?'Checked':''}}>
						...
						<div class="col-md-3">
							<div class="form-group {{ $errors->has('refno') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="refno" class="form-control" value="{{$grn->refno}}" readonly="" >
								</div>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group {{ $errors->has('trandate') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="trandate" value="{{$grn->trandate}}" class="form-control trandate" id="trandate" placeholder="Trandate" readonly="">
								</div>
							</div>
						</div>
						<div class="col-md-5">
							<div class="form-group {{ $errors->has('location') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control" name="location" id="location">
										...
										<option value="{{$loc->code}}" {{$grn->location==$loc->code?'selected':''}} >{{$loc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group {{ $errors->has('lpo') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control" name="lpo" id="lpo">
										<option value="{{$grn->lpo}}" selected="selected">{{$grn->lpo}}</option>
									</select>
								</div>
							</div>							
						</div>
							...
							<div class="form-group {{ $errors->has('account') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<select class="form-control" name="acct" id="acct">
										...
										<option value="{{$account->code}}" {{$grn->sno==$account->code?'selected':''}} >{{$account->description}}</option>
										@endforeach
									</select>
								</div>
							</div>	
						...
						<div class="col-md-8">
							<div class="form-group {{ $errors->has('remarks') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<textarea class="form-control" name="remarks" id="remarks" rows=1 style="resize: none;">{{$grn->remarks}}</textarea>
								</div>
							</div>
						</div>
						<div class="col-md-4">
									...
									<tr id="tr_{{$i}}" class="line-item">
										<td>
											<select class='form-control-grid select2-selection-grid itemcode col-xs-4' name='itemcode[]' id='itemcode_{{$i}}' style='border-radius:0px !important;width:auto;'>
												...
												<option value="{{$item->code}}" {{$grnd->icode==$item->code?'selected':''}} >{{$item->code}}</option>
												@endforeach
											</select>
										</td>
										<td>
											<select class='form-control-grid select2-selection-grid description col-xs-6' name='description[]' id='description_{{$i}}' style='border-radius:0px !important;'>
												...
												@foreach($items as $item)
												<option value="{{$item->description}}"  {{Stockspro::getItemDescr($grnd->icode)==$item->description? 'selected':''}}>{{$item->description}}</option>
												@endforeach
											</select>
											<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_{{$i}}' style='display:none;border-radius:0px !important;'>
												@foreach($items as $item)
												<option value="{{$item->bprice}}"  {{$grnd->uprice==$item->bprice?'selected':''}} >{{$item->bprice}}</option>
												@endforeach
											</select>
										...
										<td>
											<select class='form-control-grid uom  col-xs-4' name='uom[]' id='uom_{{$i}}' style='border-radius:0px !important ;width: auto' >
												...
												<option value="{{$uom->code}}" {{$grnd->punit==$uom->code?'selected':''}}>{{$uom->description}}</option>
												@endforeach
											</select>
											<select class='form-control-grid factor  col-xs-4' name='factor[]' id='factor_{{$i}}' style='display:none;border-radius:0px !important ;width: auto' >
												@foreach($units as $uom)
												<option value='{{$uom->factor}}' {{$grnd->punit==$uom->code?'selected':''}}>{{$uom->factor}}</option>
												@endforeach
											</select>
										</td>
										<td><input type="text" name="qty[]" id="qty_{{$i}}" class="text-center  form-control-grid qty  col-xs-2 " value="{{$grnd->qty}}" ></td>
										<td><input type="text" name="price[]" id="price_{{$i}}" class="form-control-grid price col-xs-2" value="{{$grnd->uprice}}" readonly></td>
										<td>
											<select class="form-control-grid vatable col-xs-6 text-danger" name="vatable[]" id="vatable_{{$i}}"'>
												<option value="1" {{$grnd->vatable==1?'selected':''}}>yes</option>
												<option value="0" {{$grnd->vatable==0?'selected':''}} >No</option>
											</select>
										</td>
										<td>
											<input type="text" id="vat_{{$i}}" value="{{$grnd->vat}}" name="vat[]" class="vat form-control-grid vat col-xs-2 text-right" readonly="">
										</td>
										<td>
											<input type="text" id="total_{{$i}}" value="{{$grnd->total}}" name="total[]" class="total form-control-grid col-xs-3 text-right" readonly="">
										</td>
										<td>
						...
						<div class="col-md-9">
							<div class="col-md-4 form-group {{ $errors->has('vattotal') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="tqty" class="tqty form-control text-right" value="{{$grn->qty}}" readonly="" >
								</div>
							</div>
							<div class="col-md-4 form-group {{ $errors->has('vattotal') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon warning">Vat Total</span>
									<input type="text" name="vattotal" class="vattotal form-control text-right" value="{{$grn->vat}}" readonly="" >
								</div>
							</div>
							<div class="col-md-4  form-group {{ $errors->has('grosstotal') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon danger">Gross Total</span>
									<input type="text" name="grosstotal" class="grosstotal form-control text-right" value="{{$grn->total}}" readonly="" >
								</div>
							</div>
...
<script type="text/javascript">
	$(function(){
		...
		function addRow(rid){
			...
			"<td>"+
			"<select class='form-control-grid select2-selection-grid itemcode col-xs-4' name='itemcode[]' id='itemcode_"+rid+"' style='border-radius:0px !important;width:auto;'>"+
			...
			"</td>"+
			"<td>"+
			"<select class='form-control-grid select2-selection-grid description col-xs-6' name='description[]' id='description_"+rid+"' style='border-radius:0px !important;'>"+
