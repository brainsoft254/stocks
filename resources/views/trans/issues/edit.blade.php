	...
	<div class="col-md-10">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="text-default"><i class="fa fa-magic "></i> <strong> Edit Issue/Transfer </strong></h3>
			</div>
			<div class="panel-body">
				<form class="form-order" action="{{route('issues.update',$issue->id)}}" method="POST">
					@csrf
					 @method('PUT')
					<div class="row">
						<div class="col-md-3">
							<div class="form-group {{ $errors->has('refno') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="refno" class="form-control" value="{{$issue->refno}}"  placeholder="SYSGEN" readonly>
								</div>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group {{ $errors->has('trandate') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="trandate" class="form-control trandate" id="trandate" placeholder="Trandate"  value="{{$issue->trandate}}" readonly="">
								</div>
							</div>
						</div>
						<div class="col-md-5">
							<div class="form-group {{ $errors->has('trantype') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon info">Transaction Type</span>
									<select class="form-control trantype" name="trantype" id="trantype">
										<option value="0" {{$issue->trantype==0?'selected':''}}>Issue</option>
										<option value="1" {{$issue->trantype>0?'selected':''}}>Transfer</option>
									</select>
								</div>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-md-7">
							<div class="form-group {{ $errors->has('client') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon acct-label info">Request No</span>
									<select class="form-control requestno" name="requestno" id="requestno">
										@foreach($requisitions as $reqs)
										<option value="{{$reqs->refno}}" {{($reqs->refno==$issue->requestno)?'selected':''}} >{{$reqs->refno}}</option>
										@endforeach
									</select>
								</div>
							</div>	
						...
						<div class="col-md-5">
							<div class="form-group {{ $errors->has('location') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon warning">Location From</span>
									<select class="form-control locationfrom" name="locationfrom" id="locationfrom">
										<option value="-1" selected="selected">--Select Location--</option>
										@foreach($locations as $loc)
										<option value="{{$loc->code}}" {{$loc->code==$issue->locfrom? "selected":''}}>{{$loc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>
						...
						<div class="col-md-7">
							<div class="form-group {{ $errors->has('remarks') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<textarea class="form-control" name="remarks" id="remarks" rows=2 style="resize: none;">{{$issue->description}}</textarea>
								</div>
							</div>
						</div>
						<div class="col-md-5">
							<div class="form-group {{ $errors->has('location') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon primary">Location To</span>
									<select class="form-control locationto" name="locationto" id="locationto">
										@foreach($locations as $loc)
										<option value="{{$loc->code}}" {{$loc->code==$issue->locto? "selected":''}}>{{$loc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>
									...
									<tr id='tr_{{$rid}}' class='line-item'>
										<td>
											<select class='form-control-grid select2-selection-grid itemcode col-xs-6' name='itemcode[]' id='itemcode_{{$rid}}' style='border-radius:0px !important;width:auto;'>
												<option value='{{$det->code}}'>{{$det->code}}</option>
											</select>
										</td>
										<td>
											...
											<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_{{$rid}}' style='display:none;border-radius:0px !important;'>
												<option value='{{$det->rate}}'>{{$det->bprice}}</option>
											</select>
										...
										<td><input type='text' name='price[]' id='price_{{$rid}}' class='form-control-grid price col-xs-2' value='{{$det->bprice}}' readonly></td>
										<td>
					...
					<div class="row">
						...
						<div class="col-md-8">
							...
							<div class="col-md-4 form-group {{ $errors->has('tqty') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon info">Qty Total</span>
									<input type="text" name="tqty" class="tqty form-control text-right" value="{{$issue->tqty}}" readonly="" >
								</div>
							</div>
							<div class="col-md-4  form-group {{ $errors->has('grosstotal') ? ' has-error' : '' }}">
								<div class="input-group">
									<span class="input-group-addon danger">Total Value</span>
									<input type="text" name="grosstotal" class="grosstotal form-control text-right" value="{{$issue->total}}" readonly="" >
								</div>
							</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('.itemcode').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'140px',dropdownParent: $('.issue-body')});
		$('.description').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'250px',dropdownParent: $('.issue-body')});
		$('.vatable,.uom,.locationfrom,.locationto,.requestno').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'100%',dropdownParent: $('body')});
		...
		function loadRequisition(v_refno)
		{
			var urli="{{url('get-requisition')}}";
			var data='refno='+v_refno;
			$.ajax({
				type: "GET",
				url: urli,
				data: data,
				...
				cache: false,
				complete:function(xhr){
					$('.issue-body').html(xhr.responseText);
					getColumnTotal();
				}
			});
		}
	...
	/*		
		...
		function addRow(rid,loadQty=false){
			if($('#locationfrom').val()==-1){swal("","Select Location From & Try AGain","error");return;}
			if($('#location').val()==-1){swal("","Select Location & Try AGain","error");return;}

			//$.ajax()
			var row="<tr id='tr_"+rid+"' class='line-item'>"+
			"<td>"+
			"<select class='form-control-grid select2-selection-grid itemcode col-xs-5' name='itemcode[]' id='itemcode_"+rid+"' style='border-radius:0px !important;width: ;'>"+
			...
			"</td>"+
			"<td>"+
			"<select class='form-control-grid select2-selection-grid description col-xs-9' name='description[]' id='description_"+rid+"' style='border-radius:0px !important;'>"+
			...
			"<option value='0' selected>0</option>"+
			@foreach($items as $item)
			"<option value='{{Stockspro::getItemQty($item->code,"+$('#locationfrom').val()+")}}'>{{Stockspro::getItemQty($item->code,"+$('#locationfrom').val()+")}}</option>"+
			@endforeach
			"</select>"+	
			...
			$('.issue-body').append(row);
			$('.itemcode').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'200px',dropdownParent: $('.form-order')});
			$('.description').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'340px',dropdownParent: $('.form-order')});
			$('.uom').select2({theme:'bootstrap',dropdownAutoWidth : true,width:'100%',dropdownParent: $('.form-order')});
		...
		$('form.form-order').on('submit', function(submission) {
			...
			if($('#locationfrom').val()==$('#locationto').val()){
				swal("",'Select Valid location From & To',"error");
				return;
			}

			swal("Update Issue/Transfer Details?", {
				buttons: {
					cancel: "Run away!",
					catch: {
						text: "Proceed!",
