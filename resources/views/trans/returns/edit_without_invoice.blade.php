	...
	<div class="col-md-10">
		<div class="box  box-primary">
			<div class="box-header">
				<h3 class="text-primary"><span class="glyphicon glyphicon-knight"></span><strong> Edit Stock Return {WITHOUT INVOICE {{{$return->id}}}}</strong><span class="ui-marker"></span></h3>
			...
			<div class="box-body">
				<form class="form-credits" action="{{route('returns.update',$return->id)}}" method="POST">
					@csrf
					@method('PUT')
						...
						<div class="row">
							<div class="col-md-4 form-group {{ $errors->has('refno') ? ' has-error' : '' }}" id="refno-group">
								<div class="input-group">
									...
									<input type="text" class="form-control" name="refno" value="{{$return->refno}}" readonly>
								</div>
							</div>
							<div class="col-md-4 form-group {{ $errors->has('trandate') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="trandate" id="trandate" class="form-control" value="{{$return->trandate}}" placeholder="" readonly>
								</div>
							</div>
							<div class="col-md-4 form-group {{ $errors->has('crinvno') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="crinvno" id="crinvno" class="form-control" value="{{$return->invno}}" placeholder="">
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-4 form-group">
								<div class="input-group">
									...
									<select class="form-control cltype" name="cltype" id="cltype">
										<option value="1" {{$return->cltype>0?'selected':''}}>Parent</option>
										<option value="0" {{$return->cltype<1?'selected':''}}>Child</option>
									</select>
								</div>
							</div>
							<div class="col-md-4"><input style="display: none;" type="text" name="rtype" id="rtype" value="0"><input type="text" name="frominvoice" id="frominvoice" class="form-control" value="0" placeholder="" readonly="" style="display:none;"></div>
							<div class="col-md-4 form-group">
								<div class="input-group">
									...
									<select class="form-control rtmode" name="rtmode" id="rtmode">
										<option value="0" {{$return->rtmode<1?'selected':''}} >Normal</option>
										<option value="1" {{$return->rtmode>0?'selected':''}} >Breakage</option>
							...
							<div class="col-md-8 form-group {{ $errors->has('clcode') ? ' has-error' : '' }}" id="clcode-group">
								<div class="input-group">
									...
									<select class="form-control clcode" name="clcode" id="clcode">
										...
										<option value="{{$client->code}}" {{$return->clcode==$client->code? 'selected':''}}>{{$client->name}}</option>
										@endforeach
									</select>
							...
							<div class="col-md-4 form-group {{ $errors->has('location') ? ' has-error' : '' }}" id="location-group">
								<div class="input-group">
									...
									<select class="form-control location" name="location" id="location">
										...
										<option value="{{$loc->code}}" {{$return->location==$loc->code? 'selected':''}} >{{$loc->description}}</option>
										@endforeach
									</select>
								</div>
							</div>	
						...
						<div class="row">
							<div class="col-md-12 form-group {{ $errors->has('remarks') ? ' has-error' : '' }}">
								<div class="input-group">
									...
									<input type="text" name="remarks" id="remarks" class="form-control" value="{{$return->remarks}}" >
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class = "col-md-12">
								<table class="table-items" style="width:100%;" >
									...
									<?php $i=1; $tqty=0;?>
									@foreach($return->return_details as $returnd)
									<tr id="tr_{{$i}}" class="line-item">
										<td>
											<select class='form-control-grid select2-selection-grid icode col-xs-4' name='icode[]' id='icode_{{$i}}' style='border-radius:0px !important;width:auto;'>
												

												<option value="{{$returnd->icode}}" selected >{{$returnd->icode}}</option>
												
											</select>
										</td>
										<td>
											<select class='form-control-grid select2-selection-grid idesc col-xs-6' name='idesc[]' id='idesc_{{$i}}' style='border-radius:0px !important;'>
												
												
												<option value="{{Stockspro::getItemDescrClient($returnd->icode,$return->clcode)}}"  selected>{{Stockspro::getItemDescrClient($returnd->icode,$return->clcode)}}</option>
												
											</select>
											<select class='form-control-grid iprice col-xs-6' name='iprice[]' id='iprice_{{$i}}' style='display:none;border-radius:0px !important;'>
												
												<option value="{{Stockspro::getClientPrice($returnd->icode,$return->clcode)}}"  selected >{{Stockspro::getClientPrice($returnd->icode,$return->clcode)}}</option>
												
											</select>
											<select class='form-control-grid iqty col-xs-6' name='iqty[]' id='iqty_{{$i}}' style='display:none;border-radius:0px !important;'>

												<option value='{{Stockspro::getItemQty($returnd->icode,$return->location)}}' selected >
													{{ Stockspro::getItemQty($returnd->icode,$return->location) }}
												</option>
										...
										<td>
											<select class='form-control-grid uom  col-xs-4' name='uom[]' id='uom_{{$i}}' style='border-radius:0px !important ;width: auto' >
												...
												<option value="{{$returnd->uom}}" {{$returnd->uom==$uom->code?'selected':''}}>{{$uom->description}}</option>
												@endforeach
											</select>
											<select class='form-control-grid factor  col-xs-4' name='factor[]' id='factor_{{$i}}' style='display:none;border-radius:0px !important ;width: auto' >
												@foreach($units as $uom)
												<option value='{{$uom->factor}}' {{$returnd->uom==$uom->code?'selected':''}}>{{$uom->factor}}</option>
												@endforeach
											</select>
										</td>
										<td><input type="text" name="qty[]" id="qty_{{$i}}" class="text-center  form-control-grid qty  col-xs-2 " value="{{$returnd->qty}}" ></td>
										<td><input type="text" name="rate[]" id="rate_{{$i}}" class="form-control-grid rate col-xs-2" value="{{$returnd->rate}}" readonly></td>
										<td>
											<select class="form-control-grid vatable col-xs-6 text-danger" name="vatable[]" id="vatable_{{$i}}"'>
												<option value="1" {{$returnd->vat>0?"selected":""}}>yes</option>
												<option value="0" {{$returnd->vat==0?"selected":""}}>No</option>
											</select>
										</td>										
										<td>
											<input type="text" id="vat_{{$i}}" value="{{$returnd->vat}}" name="vat[]" class="vat form-control-grid vat col-xs-2 text-right" value="{{$return->tax}}" readonly="">
										</td>
										<td>
											<input type="text" id="total_{{$i}}" value="{{$returnd->total}}" name="total[]"  value="{{$returnd->total}}" class="total form-control-grid col-xs-3 text-right" readonly="">
										</td>
										<td>
							...
							<div class="col-md-8">
								<div class="col-md-4 form-group {{ $errors->has('vattotal') ? ' has-error' : '' }}">
									<div class="input-group">
										...
										<input type="text" name="tqty" class="tqty form-control text-right" value="{{$tqty}}" readonly="" >
									</div>
								</div>
								<div class="col-md-4 form-group {{ $errors->has('gvat') ? ' has-error' : '' }}">
									<div class="input-group">
										<span class="input-group-addon warning">Vat Total</span>
										<input type="text" name="gvat" class="gvat form-control text-right" value="{{$return->vat}}" readonly="" >
									</div>
								</div>
								<div class="col-md-4  form-group {{ $errors->has('gtotal') ? ' has-error' : '' }}">
									<div class="input-group">
										<span class="input-group-addon danger">Gross Total</span>
										<input type="text" name="gtotal" class="gtotal form-control text-right" value="{{$return->amount}}" readonly="" >
									</div>
								</div>
...
<script type="text/javascript">
	$(function(){
		$('.clcode,.cltype,.rtmode,.location').select2({theme:'bootstrap',closeOnSelect: true,width:'100%',dropdownParent: $('.box-body')});
		...
		function addRow(rid,loadQty=false){
			...
			"<td><input type='text' name='rate[]' id='rate_"+rid+"' class='form-control-grid rate col-xs-2' value='0' readonly></td>"+
			"<td>"+
