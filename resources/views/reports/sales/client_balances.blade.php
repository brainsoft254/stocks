				...
				<div class="row">
					<div class="col-md-12">
							<table class="tbl-main p-5">
								<thead class="bg-primary" style="margin-bottom:4px;">
									<tr>
										<th>#</th>
										<th>Code</th>
										<th>Name</th>
								...
								<tbody>
									...
									@if($cltype==1 && $summary!=1 && $client==-1)
										@foreach($parents->sortBy('code') as $parent)
											<?php $total=0;$i=1;?>
											
											<tr>
												<td colspan="4"  ><hr/><div class="label-primary" style="width:100% ;padding:5px; font-style:bold">{{$parent->name}}</div></td>
											</tr>
											@foreach($details->sortBy('parent') as $det)
											@if($det->parent==$parent->code )
											...
											@endforeach
											<tr>
												<td colspan="3" class="bg-secondary text-right"><b>Sub Total</b></td>
												<td class="bg-danger text-right"><b>{{number_format($total,2)}}</b></td>
											</tr>
										@endforeach
									@else
										@foreach($details->sortBy('parent') as $det)
								...
								<tfoot class="text-danger">
									<tr>
										...
										<td colspan="1" class="text-right"><strong>{{number_format($totalBals,2)}}</strong></td>
										<td></td>
									</tr>
								</tfoot>
							</table>
...
<script type="text/javascript">
	$(function(){
		$(document).on('click','#btnPrintPdf',function(e){
			...
			var datastr='&asatdate={!!$asatdate!!}&cltype={!!$cltype!!}&summary={!!$summary!!}&client={!!$client!!}';
			var urli=$(this).attr('href')+'?pdf=1'+datastr;

			//swal({html:'<iframe src="'+urli+'"  width="800" height="900" class="xl"></iframe>'});
