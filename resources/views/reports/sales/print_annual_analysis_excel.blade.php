...
<div class="section">
	<div class="row">
		<div class="col-md-12">
                <table class="tbl-params" style="width:30%">
                    <tr><td><strong>Year:</strong></td><td class="text-primary"><b>{{$period}}</b></td></tr>
                    <tr><td><b>Data Source:</b></td><td class="text-danger"><b>{{$datasource}}</b></td></tr>
                    <tr><td><b>Report Type:</b></td><td class="text-success"><b>{{$rpt_type}}</b></td></tr>
                    <tr><td><b>Client:</b></td><td class="text-success"><b>{{$client}}</b></td></tr>
                    {{-- <tr><td><b>Total Sales:</b></td><td class="text-danger bg-danger text-c"><b>{{number_format($totalsales,0)}}</b></td></tr> --}}
                </table>
				<div class="row">
					<div class="col-md-12">
							<table class="tbl-main p-5">
								<thead class="bg-primary" style="margin-bottom:4px; border-bottom:solid 1px #000">
								<tr>
                                        <th>#</th>
                                        <th>Code</th>
                                        <th>Name</th>
								...
								<tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($monthsBtn as $key=> $month)
                                         ...
                                         <td class="text-right {{$msale>0? 'text-success':'text-danger'}} "><b>{{number_format($msale)}}</b></td>
                                         <?php 
										    switch ($month) {
												case 'Jan':
													$jantotal+=$msale;
										 ...
										 ?>
                                         @endforeach
                                         <td class="text-right text-primary {{$rtotal>0? 'text-success':'text-danger'}}"><b class="text-primary">{{number_format($rtotal,0)}}</b></td>
                                         <td class="text-right {{$det->crnote>0? 'text-warning':'text-default'}}"><b>{{number_format($det->crnote,0)}}</b></td>
                                         <td class="text-right {{$det->paid>0? 'text-success':'text-default'}}"><b>{{number_format($det->paid,0)}}</b></td>
                                         <td class="text-right {{$det->paid>0? 'text-success':'text-default'}}"><b>{{number_format($rtotal-$det->crnote-$det->paid,0)}}</b></td>
                                         <?php 
										 $coltotal+=$rtotal;
										 $crtotal+=$det->crnote;
										 $paidtotal+=$det->paid;
								...
								<tfoot class="text-success">
									<tr>
										...
										<td colspan="1" class="text-right"></td>
                                        <td class="text-right" style='color:blue'><strong>{{number_format($coltotal,0)}}</strong></td>
										<td colspan="1" class="text-right"></td>
									...
									<tr>
										...
										<td colspan="1" class="text-right"></td>
										<td class="text-right" style='color:red'><strong>{{number_format($crtotal,0)}}</strong></td>
										<td colspan="1" class="text-right"></td>
