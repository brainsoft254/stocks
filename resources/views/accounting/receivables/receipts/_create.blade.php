...
<script type="text/javascript">
	$(function(){
		...
		function loadInvoices(_vatx){
			...
			console.log('startDate=',$.datepicker({format:'yyyy-mm-dd'},Date.parse($('#datefrom').val())));
			...
			for(i in items){
				if($('#client').val()==($('#parent_check').prop("checked")?items[i].parent:items[i].clcode)){
					...
					if(DateBetween(iDate,startDate,endDate)){
						...
						"<input type='text' name='clcodex[]' id='clcodex_"+i+"' class='text-left form-control-grid clcodex col-xs-2' value='"+items[i].clcode+"' readonly style='display:none'>"+
						"</td>"+
						"<td style='width:15% !important' >"+
						"<input type='text' name='due[]' id='due_"+i+"' class='text-right form-control-grid due  col-xs-2 bg-primary' value='"+$.number(due,2)+"' readonly>"+
						"</td>"+
						"<td style='width:20% !important'>"+
						"<input type='text' name='paid[]' id='paid_"+i+"' class='text-right form-control-grid paid  col-xs-2 ' value='0.00' focus >"+
						"</td>"+
						...
						*/						
						...
						$('#lpo').prop('disabled', false);
					        ...
					        $.ajax({
					        	type: "POST",
					        	url: url,
					        	data: data+"&totalpaid="+$('.totalpaid').val()+
